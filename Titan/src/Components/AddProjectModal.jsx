...
const AddProjectModal = ({ onClose, onProjectAdded }) => {
  ...
  useEffect(() => {
    const fetchEmployees = async () => {
      try {
        ...
        showErrorToast('Unable to load employees');
      }
    };
    fetchEmployees();
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showErrorToast(err.response?.data?.message || 'Failed to add project');
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-zinc-900 p-6 rounded-xl shadow-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <h2 className="text-xl font-semibold mb-5 text-accent-400">Add New Project</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className={labelClass}>Project Name *</label>
              <input name="name" placeholder="Enter project name" value={formData.name} onChange={handleChange} className={inputClass} required />
            </div>
            <div>
              <label className={labelClass}>Client *</label>
              <input name="client" placeholder="Enter client name" value={formData.client} onChange={handleChange} className={inputClass} required />
            </div>
          </div>
          ...
          <div>
            ...
            {formData.projectHead && (
              <p className="text-xs text-accent-400 mt-1">
                ★ This person will be auto-added to the team and can update progress.
              </p>
            )}
          </div>
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <button type="submit" className="px-4 py-2 bg-accent-600 hover:bg-accent-700 rounded-md">Add Project</button>
          </div>
        </form>
      </div>
    </div>
